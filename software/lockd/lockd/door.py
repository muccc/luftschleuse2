...
class Door:
    ...
    def read_rx_sequence_number_from_container(self):
        ...
        try:
            ...
            self.logger.error(
                "%s: Could not open rx sequence number \
                                container at %s"
        ...
        if config is not None:
            ...
            self.logger.error(
                "%s: Could not read rx sequence number \
                            from container at %s"
                % (self.name, self.config_file)
            )
    ...
    def update(self, message):
        ...
        if not p.seq >= self.min_rx_seq:
            self.logger.debug(
                "%s: Seq %d not ok. Sending seq update to %d."
            ...
            self.interface.writeMessage(
                self.priority,
                bytes(self.address, encoding="ascii"),
                msg,
                self.tx_msg_queue,
                0.02,
            )
        ...
        if p.cmd == ord("S"):
            ...
            doorstate = p.data[1]
            self.closed = doorstate & Door.DOOR_CLOSED == Door.DOOR_CLOSED
            self.locked = doorstate & Door.LOCK_LOCKED == Door.LOCK_LOCKED
            self.locking = doorstate & Door.LOCK_LOCKING == Door.LOCK_LOCKING
            self.unlocked = doorstate & Door.LOCK_UNLOCKED == Door.LOCK_UNLOCKED
            self.unlocking = doorstate & Door.LOCK_UNLOCKING == Door.LOCK_UNLOCKING
            self.handle_pressed = doorstate & Door.HANDLE_PRESSED == Door.HANDLE_PRESSED
            self.manual_unlocked = (
                doorstate & Door.LOCK_MANUAL_UNLOCKED == Door.LOCK_MANUAL_UNLOCKED
            )
            self.logger.info("%s: Door state: %s" % (self.name, self.get_state()))
            self.logger.info(
                "%s: Desired door state: %s" % (self.name, self.get_desired_state())
            )
    ...
    def tick(self):
        if time.time() > self.periodic_timeout:
            self.periodic_timeout = time.time() + 1 / 3.0
