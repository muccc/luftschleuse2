...
class Display:
    ...
    def _command(self, cmd, data=[]):
        msg = bytes([cmd] + data)
        self._interface.writeMessage(
            self.priority, bytes([0xFE]), msg, self.tx_msg_queue
        )
    ...
    def _stream_raw(self, raw_data):
        for i in range(0, len(raw_data), 120):
            self._interface.writeMessage(
                self.priority, bytes([0xFD]), raw_data[i : i + 120], self.tx_msg_queue
            )
