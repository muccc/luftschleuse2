...
class Lockd:
    def __init__(self, config):
        ...
        self.serial_interface = SerialInterface(serialdevice, baudrate, timeout=0.1)
        ...
        for section in config.sections():
            if config.has_option(section, "type"):
                ...
                if t == "door":
                    ...
                    door_address = bytes(
                        config.get(door_name, "address"), encoding="ascii"
                    )
                ...
                self.master = MasterController(
                    self.serial_interface, self.input_queue, buttons, leds
                )
    ...
    def process(self):
        ...
        while not timeout:
            ...
            if address != False:
                self.logger.debug(
                    "Received data from address %s :%s" % (address, str(list(message)))
                )
            else:
                timeout = True
...
def main():
    ...
    handler = logging.handlers.SysLogHandler(
        address=(logger_host, logger_port), facility=19
    )
